...
* Run:
  ...
  in order to use the docker daemon inside the minikube cluster
...
running in GKE I decided to just go for it and attempt the step 2. For step 2, I used Terraform and the material mentioned in the section "Learning resources" to
setup a automate the creation/management of the infrastructure of Document keeper. 

## Improvements list
In this section, I will be listing the improvements I would like to perform on this assignement:
...
3) Implement a way for Terraform not to consider the creation of the GKE cluster as failed due to timeout when the in the GKE UI the cluster is still updating and after some time finishes with sucess.

4) Define a release strategy for the images with version management.

## Feedback
Feedback on improvement points, tips to implement the improvement list or just new ideas that can make this service better, more reliable, secure and performatic are always welcome and will be discussed and iterated upon. Therefore, feel free to reach out to me either on the CNCF slack channel or via Linkedin. :) 
